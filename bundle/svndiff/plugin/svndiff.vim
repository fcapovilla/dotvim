...
function s:Svndiff_update(...)
    ...
    exec 'sign place 999999 line=1 name=svndiff_start file=' . fname
...
function Svndiff(...)
	...
	if cmd == 'clear'
		...
		if exists("s:is_active[fname]") 
			unlet s:is_active[fname]
			exec 'sign unplace 999999 file=' . fname
		endif
		call s:Svndiff_clear()
	end
...
sign define svndiff_start  text=o texthl=diffAdd


" Define autocmds if autoupdate is enabled

if exists("g:svndiff_autoupdate")
	"autocmd CursorMoved * call s:Svndiff_update()
